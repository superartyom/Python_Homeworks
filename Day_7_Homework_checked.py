...
#     if not str_4:
#         return ''
#     return str_4 + str(list(str_4).pop(-1)) + rev(str_4)  # վերջին տառը կարող ենք վերցնել str_4[-1]
# print(rev("lakiaon"))
...
# def srt(lst_6):  # sort անվամբ reserved keyword չկա, կարող ենք հանգիստ օգտագործել
#     for x in range(1, len(lst_6)):
#         if lst_6[x] < lst_6[x-1]:
#             t = lst_6[x]
#             print(lst_6)
#             lst_6 = list(lst_6[0:x]) + list(lst_6[x+1:-1]) + list(lst_6[-1])  # list(int)ը չի աշխատում։ Երրորդ գումարելիդ int է
...
# Թղթի վրա եթե նկարես, կհասկացվի։ Օրինակ եթե շրջանագծերի կենտրոնների հեռավորությունը ավելի մեծ է քան դրանց շառավղերի
# գումարը, ուրեմն չեն հատվում ու հեռու են իրարից։ Էդպես կարելի է նաև մյուս պայմանները ստուգել
...
for k, v in dict_8_1.items():
    for k2, v2 in dict_8_2.items():
        if k == k2:
            ...
            dict_8_3.update({k2: v2})  # Որովհետև else-ի մեջ կմտնի նաև էն դեպքում երբ k = 5, իսկ k2 = 7: Ու մեկ ա update
            # կանի արդեն ճիշտ արժեքը։ Ավելի ճիշտ կլինի ստուգես if k in dict_8_2 կամ նման մի բան
print(dict_8_3)
