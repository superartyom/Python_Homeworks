...
lst_1 = []
lst_1 = [x * x for x in lst_1]  # Էսպես չի լինի։ Դատարկ լիստ ա ստացվում: Համ էլ պետք է թիվ ստանանք, ոչ թե լիստ։
...
# Արտյոմ ջան, տեստ արա կոդերդ լիստերի հետ։ Մի հատ լիստ սահմանիր, որը սթրինգերից ա բաղկացած ու դրա վրա գործածի կոդդ
tot_2 = 0
lst_2 = []
for str_2 in lst_2:
...
# Նույնն էլ այստեղ։ Տեսային լիստեր են պետք
lst_6 = []
checc_6 = False
...
for x in mdict.keys():
    if x == inkey:
        Checc_8 = True
if not checc_8:  # Կարելի էր ուղղակի else գրել նախորդ if-ի համար։ Այդ դեպքում checc_8 փոփոխականի կարիքը չէր լինի
...
mdict_9 = {'key': 'Value'}
lst_9 = [n9 for n9 in range(len(mdict_9.values()) - 1)]  # ինչի՞ range(): Այսպես ոչ թե արժեքները կավելացնենք, այլ
# ինդեքսները։ Պարզապես [n for n in dict.values()]
